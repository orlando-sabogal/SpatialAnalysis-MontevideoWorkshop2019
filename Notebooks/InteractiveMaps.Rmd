...
Originallyl, Leaflet is a javascript [library](https://leafletjs.com/). If yoy know your way around **js** it goes something like this: 
...
To a create a leaflet object you only have to call the **leaflet()** function. If you have an object (think about an *sf*) which you want to plot you should pass it through the function as **leaflet(YourObject)**. To add layers the **%>%** is used before a new function defining a new characteristic of the map. 
As a backgroung layer leaflet enables to use different maps providers with **addProviderTiles.**
...
In the previous map you can zoom in and zoom out all aorund the world. You can set the specific view you want with the **setView()** function as in the following examples where the maps are centered in *Plaza de la indepencia* (Montevideo, Uruguay)
