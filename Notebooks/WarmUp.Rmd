...
```{r}
Zones %>% select(AREA_KM2_) %>% summary()
```
...
```{r}
BiggesStates <- Zones %>% filter(AREA_KM2_ > 13000)
```

```{r}
#Zones <- Zones %>% mutate(Uruguay = 1)
#Uruguay <- Zones %>% group_by(Uruguay) %>% summarise(CantidadEstados <- n())
#Uruguay <- Zones %>% st_cast(to="POLYGON")
```
...
```{r}
Test <- st_join(Accidents,Zones)
```

```{r}
st_crs(Accidents) == st_crs(Zones) 
Zones <- Zones %>% st_set_crs(32721)
st_crs(Accidents) == st_crs(Zones) 
```
